...
evalClust <- function(sym.matrix){
  ...
  sym.matrix <- as.sym.matrix( get.adjacency(g, attr = "weight"))
    ...
    sym.matrix <- as.data.frame(sym.matrix)
    diag(sym.matrix)        <- 0
    sym.matrix[sym.matrix < 0] <- 0
  ...
  VI<-matrix(,nrow = 50, ncol = length(percent))
  ARI<-matrix(,nrow = 50,ncol = length(percent))
  modularity.value <-matrix(,nrow = 50, ncol = length(percent))
  ...
  for(k in 2:length(percent)) {
    for(p in 1:10){ # for each degree of perturbation run 100 times
      new.v <- as.matrix(rewirematrix(sym.matrix, percent[k]))
      new.g                  <- graph.adjacency(as.matrix(new.v), mode = "undirected", weighted = TRUE)
      clust.sol       <-  walktrap.community(new.g, weights = E(g)$weight, steps = 4)
  ...
  VI.rando<-matrix(,nrow = 10, ncol = length(percent))
  ARI.rando<-matrix(,nrow = 10,ncol = length(percent))
  modularity.rando <-matrix(,nrow = 10, ncol = length(percent))
  ...
  plotARI <-  points(percentlab, colMeans(ARI[1:10,]), col = "black") + lines(percentlab, rep10ari) + lines(percentlab, rep20ari)
  ...
  plotVI <- plotVI + points(percentlab, colMeans(VI[1:10,]), col = "black") + lines(percentlab, rep10vi) + lines(percentlab, rep20vi)
